...
from reformat.parse_shebang import normalize_cmd
...
def _run_legacy(
    ...
    if os.environ.get('PRE_COMMIT_RUNNING_LEGACY'):
        raise SystemExit(
            f"bug: reformat's script is installed in migration mode\n"
            f'run `reformat install -f --hook-type {hook_type}` to fix '
            f'this\n\n'
            f'Please report this bug at '
            f'https://github.com/reformat/reformat/issues',
        )
...
def _validate_config(
    ...
    if not os.path.isfile(config):
        if skip_on_missing_config or os.getenv('PRE_COMMIT_ALLOW_NO_CONFIG'):
            print(f'`{config}` config file not found. Skipping `reformat`.')
            ...
            print(
                ...
                f'- To permanently silence this, install reformat with the '
                f'--allow-missing-config option\n'
                f'- To uninstall reformat run `reformat uninstall`',
            )
            raise SystemExit(1)
...
def _run_ns(
    ...
    elif hook_type in {'post-commit', 'pre-merge-commit', 'reformat'}:
