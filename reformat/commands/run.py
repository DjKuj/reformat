...
from reformat.repository import all_hooks
from reformat.repository import install_hook_envs
from reformat.staged_files_only import staged_files_only
...
def _run_hooks(
    ...
    if retval and args.show_diff_on_failure and prior_diff:
        if args.all_files:
            output.write_line(
                'reformat hook(s) made changes.\n'
                'If you are seeing this message in CI, '
                'reproduce locally with: `reformat run --all-files`.\n'
                'To run `reformat` as part of git workflow, use '
                '`reformat install`.',
            )
...
def run(
    ...
    if stash and _has_unstaged_config(config_file):
        logger.error(
            f'Your reformat configuration is unstaged.\n'
