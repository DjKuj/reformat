...
from reformat.languages.all import languages
from reformat.languages.helpers import environment_dir
...
def _hook(
    ...
    version = ret['minimum_reformat_version']
    if parse_version(version) > parse_version(C.VERSION):
        logger.error(
            f'The hook `{ret["id"]}` requires reformat version {version} '
    ...
    for hook in repo_config['hooks']:
        if hook['id'] not in by_id:
            logger.error(
                ...
                f'Often `reformat autoupdate` fixes this.',
            )
            exit(1)
