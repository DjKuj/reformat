...
from reformat.color import add_color_option
from reformat.commands.autoupdate import autoupdate
from reformat.commands.clean import clean
from reformat.commands.gc import gc
from reformat.commands.hook_impl import hook_impl
from reformat.commands.init_templatedir import init_templatedir
from reformat.commands.install_uninstall import install
from reformat.commands.install_uninstall import install_hooks
from reformat.commands.install_uninstall import uninstall
from reformat.commands.migrate_config import migrate_config
from reformat.commands.run import run
from reformat.commands.sample_config import sample_config
from reformat.commands.try_repo import try_repo
from reformat.error_handler import error_handler
from reformat.logging_handler import logging_handler
from reformat.store import Store


logger = logging.getLogger('reformat')

# https://github.com/reformat/reformat/issues/217
...
def _add_hook_type_option(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        '-t', '--hook-type', choices=(
            'reformat', 'pre-merge-commit', 'pre-push', 'prepare-commit-msg',
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    autoupdate_parser = subparsers.add_parser(
        'autoupdate',
        help="Auto-update reformat config to the latest repos' versions.",
    )
    ...
    clean_parser = subparsers.add_parser(
        'clean', help='Clean out reformat files.',
    )
    ...
    init_templatedir_parser.add_argument(
        ...
        help='Assume cloned repos should have a `reformat` config.',
    )
    _add_hook_type_option(init_templatedir_parser)

    install_parser = subparsers.add_parser(
        'install', help='Install the reformat script.',
    )
    ...
    install_parser.add_argument(
        ...
        help=(
            'Whether to allow a missing `reformat` configuration file '
    ...
    install_hooks_parser = subparsers.add_parser(
        ...
        help=(
            ...
            'file.  You may find `reformat install --install-hooks` more '
            'useful.'
        ),
    )
    ...
    uninstall_parser = subparsers.add_parser(
        'uninstall', help='Uninstall the reformat script.',
    )
