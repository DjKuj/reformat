...
from reformat.languages.all import all_languages
from reformat.logging_handler import logging_handler
from reformat.util import parse_version
from reformat.util import yaml_load
...
def check_type_tag(tag: str) -> None:
    if tag not in ALL_TAGS:
        raise cfgv.ValidationError(
            ...
            f'Try upgrading identify and reformat?',
        )
...
def check_min_version(version: str) -> None:
    if parse_version(version) > parse_version(C.VERSION):
        raise cfgv.ValidationError(
            f'reformat version {version} is required but version '
            f'{C.VERSION} is installed.  '
            f'Perhaps run `pip install --upgrade reformat`.',
        )
...
MANIFEST_HOOK_DICT = cfgv.Map(
    ...
    cfgv.Optional('minimum_reformat_version', cfgv.check_string, '0'),
        ...
        if self.key in dct:
            ...
            if '.' not in rev and not re.match(r'^[a-fA-F0-9]+$', rev):
                logger.warning(
                    ...
                    f'See https://reformat.com/#using-the-latest-version-for-a-repository '  # noqa: E501
                    f'for more details.  '
                    f'Hint: `reformat autoupdate` often fixes this.',
                )
...
class OptionalSensibleRegexAtHook(cfgv.OptionalNoDefault):
    def check(self, dct: Dict[str, Any]) -> None:
        ...
        if r'[\/]' in dct.get(self.key, ''):
            logger.warning(
                fr'reformat normalizes slashes in the {self.key!r} field '
...
class OptionalSensibleRegexAtTop(cfgv.OptionalNoDefault):
    def check(self, dct: Dict[str, Any]) -> None:
        ...
        if r'[\/]' in dct.get(self.key, ''):
            logger.warning(
                fr'reformat normalizes the slashes in the top-level '
...
def _entry(modname: str) -> str:
    ...
    """
    return f'{shlex.quote(sys.executable)} -m reformat.meta_hooks.{modname}'
...
CONFIG_SCHEMA = cfgv.Map(
    ...
    # do not warn about configuration for reformat.ci
...
def ordered_load_normalize_legacy_config(contents: str) -> Dict[str, Any]:
    ...
    if isinstance(data, list):
        logger.warning(
            'normalizing reformat configuration to a top-level map.  '
            'support for top level list will be removed in a future version.  '
            'run: `reformat migrate-config` to automatically fix this.',
        )
        return {'repos': data}
    else:
        return data
